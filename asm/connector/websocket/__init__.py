...
class ConnectorWebsocket(Connector):
    ...
    def __init__(self, config, asm=None):
        ...
        self.bot_name = self.config.get("abot-name", 'abot')

    async def connect(self):
        """Connect to the chat service."""
        self.accepting_connections = True

        self.asm.web_server.web_app.router.add_get(
            "/connector/websocket/{socket}",
            self.websocket_handler)

        self.asm.web_server.web_app.router.add_post(
            "/connector/websocket",
    ...
    async def websocket_handler(self, request):
        ...
        async for msg in websocket:
            if msg.type == aiohttp.WSMsgType.TEXT:
                message = Message(msg.data, "guest", None, self)
                await self.bot.parse(message)
