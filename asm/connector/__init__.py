...
from asm.utils.events import Event, Reaction, Message
...
def register_event(event_type):
    """
    Register a method to handle a specific `arcus.events.Event` object.
    Args:
        event (Event): The event class this method can handle.
    """

    def decorator(func):
        if hasattr(func, "__arcus_events__"):
            func.__arcus_events__.append(event_type)
        else:
            func.__arcus_events__ = [event_type]
        return func
...
class Connector:
    """A base connector.
    Connectors are used to interact with a given service.
    """
    ...
    def __new__(cls, *args, **kwargs):
        ...
        # Filter out anything that's not got the attribute __arcus_event__
        event_methods = filter(
            lambda f: hasattr(f, "__arcus_events__"),
        ...
        for event_method in event_methods:
            for event_type in event_method.__arcus_events__:
                if not issubclass(event_type, Event):
                    err_msg = (
                        "The event type {event_type} is "
                        "not a valid Arcus event type"
                    )
    ...
    async def connect(self):
        ...
        This method should create a connection to the desired service.
    ...
    async def listen(self):
        ...
        Message objects for messages coming from the service. It should
        then call `await service.parse(message)` on those messages.
    ...
    async def respond(self, message, room=None):
        ...
        back to the service. It may also have a `room` and `user` property
    ...
    async def disconnect(self):
        ...
        This method is called when the service is exiting, it can be used to close
    ...
    async def send(self, event):
        ...
        if not isinstance(event, Event):
            raise TypeError(
                "The event argument to send must be an arcus Event object"
            )
