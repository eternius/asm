...
from asm.utils.events import Message, Reaction
from asm.connector.slack.events import Blocks
...
class ConnectorSlack(Connector):
    ...
    def __init__(self, config, asm=None):
        ...
        self.websocket = None
        self.bot_name = config.get("bot-name", "bot")
        self.known_users = {}
        self.keepalive = None
    ...
    async def connect(self):
        ...
        try:
            ...
            if self.keepalive is None or self.keepalive.done():
                self.keepalive = self.asm.eventloop.create_task(
