"""Submodule to handle web requests in service."""
...
class Web:
    """Create class for service Web server."""

    def __init__(self, service):
        """Create web object."""
        self.service = service
        try:
            self.config = self.service.config["web"]
        except KeyError:
            self.config = {}
    ...
    def build_response(status, result):
        """Build a json response object to power the service reponses.
        Args:
            result: serialize obj as a JSON formated stream
        Returns:
            json: returns json object with list of responses for the service
        """
    ...
    def setup_webhooks(self, modules):
        """Add the webhooks for the webhook modules to the router."""
        for module in modules:
            for matcher in module.matchers:
                if "webhook" in matcher:
                    self.register_module(self.service, module, matcher["webhook"])

    async def web_index_handler(self, request):
        """Handle root web request to service API.
        Args:
            request: web request to the root (index)
        Returns:
            dict: returns successful status code and greeting for the root page
        """
        return self.build_response(200, {"message": "Welcome to the arcus API"})

    async def web_stats_handler(self, request):
        """Handle stats request.
        Args:
            request: web request to render service stats
        Returns:
        ...
        return self.build_response(
            200,
            {
                ...
                "modules": {
                    "skills": len(self.service.modules),
                    "connectors": len(self.service.connectors),
                    "databases": len(self.service.memory.databases),
                },
            },
        )
